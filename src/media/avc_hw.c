...
extern int sceMpegGetAvcNalAu(SceMpeg *, GoAvcNal *, SceMpegAu *);
extern int sceMpegAvcDecodeDetail2(SceMpeg *, GoAvcDetail2 **);
                   ...
                   int width, int height)
{
    ...
    return 0;
fail:
    go_modern_trace("AVC_HW init failed result=%08x", result);
    go_avc_hw_shutdown();
    return -1;
}
                     ...
                     const unsigned char **rgba, int *stride)
{
    ...
    result = sceMpegGetAvcNalAu(&hw.mpeg, &nal, hw.au);
    if (result < 0) goto fail;
    result = sceMpegAvcDecode(&hw.mpeg, hw.au, 512, NULL, &pictures);
    if (result < 0) goto fail;
    hw.first_sample = 0;
    if (pictures <= 0) return 0;
    result = sceMpegAvcDecodeDetail2(&hw.mpeg, &detail);
    if (result < 0 || !detail || !detail->info || !detail->yuv) goto fail;
