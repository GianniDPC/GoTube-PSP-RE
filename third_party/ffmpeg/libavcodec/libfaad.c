                             ...
                             uint8_t *buf, int buf_size)
{
    ...
    /* FAAD2 may consume an AAC priming/configuration packet without returning
     * PCM.  The historical wrapper dereferenced NULL here, killing the audio
     * worker before the video queue could advance. */
    if (!out || frame_info.samples == 0) {
        if (data_size)
            *data_size = 0;
