...
/* These offsets are read directly by GoTube's native FUN_0000dc04,
 * FUN_0000de68 and FUN_000104b8.  They also distinguish 0.22 from later,
 * incompatible intraFont layouts. */
typedef char gt_if022_size_offset[(offsetof(intraFont, size) == 0x34) ? 1 : -1];
typedef char gt_if022_color_offset[(offsetof(intraFont, color) == 0x38) ? 1 : -1];
typedef char gt_if022_shadow_offset[(offsetof(intraFont, shadowColor) == 0x3c) ? 1 : -1];
typedef char gt_if022_options_offset[(offsetof(intraFont, options) == 0x40) ? 1 : -1];
...
float intraFontPrintUCS2(intraFont *font, float x, float y, const unsigned short *text) {
	...
	/* GoTube's embedded FUN_000104b8 submits GU-list vertices directly; its
	 * native tail contains neither this later defensive writeback nor depth
	 * state toggles.  sceGuGetMemory already returns list-owned storage. */
