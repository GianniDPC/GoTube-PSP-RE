...
float intraFontPrint(intraFont *font, float x, float y, const char *text) {
  if (!font) return x;
  int codepage = (font->options & INTRAFONT_STRING_SJIS_OLD) ? CCC_CP932 : CCC_CP000;
  int length = cccStrlenCode((cccCode*)text, codepage);
...
#define LOCAL_BUFFER_LENGTH 256
  ...
  length = cccCodetoUCS2(ucs2_text, length, (cccCode*)text,
                         (font->options & INTRAFONT_STRING_SJIS_OLD) ? CCC_CP932 : CCC_CP000);
...
float intraFontPrintColumnUCS2Ex(intraFont *font, float x, float y, float column, const cccUCS2 *text, int length) {
  ...
  // GoTube's pre-0.25 ABI has no scrolling mode; 0x2000 selects Shift-JIS.
  int i;
  if (0) {
    for (i = 0; i < length; i++) {
      if (text[i] == '\n') {
  ...
  for (i = 0; i < length; i++) {
    ...
    if ((i == 0) || (text[i] == '\n') || ((column > 0.f) && (i >= eol) && (text[i] != 32))) { //newline
      ...
      if (column > 0.f) {                                                
        if (0) { // scrolling did not exist in the ABI embedded by GoTube
          eol = length;
          scroll = 1;
          left = ((float)((int)x));
  ...
  /* GoTube's embedded pre-0.25 renderer submits the glyph array without
   * changing depth state (native FUN_000104b8 tail).  The later 0.31 toggle
   * left depth enabled after each text call and changed all subsequent 2-D
   * rendering in the frame. */
