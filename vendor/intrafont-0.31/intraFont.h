...
/* GoTube embeds the pre-scrolling intraFont ABI.  In that ABI bit 0x2000 is
 * the Shift-JIS input selector, not INTRAFONT_SCROLL_LEFT (introduced later
 * in intraFont 0.25).  Native FUN_0001172c tests this exact bit. */
#define INTRAFONT_STRING_SJIS_OLD  0x00002000
/* Kept only so the later column-renderer source still compiles.  All entry
 * tests are disabled in the GoTube compatibility build. */
...
#define PGF_SCROLL_MASK   0x00000000
#define PGF_CACHE_MASK    0x0000C000
#define PGF_STRING_MASK   0x00002000
